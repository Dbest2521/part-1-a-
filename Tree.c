...
//line 16 is added because it have to have a list of urls
#include <stdlib.h>
#include <stdio.h>
#include <assert.h>
#include <string.h>
...
static Link searchSplay(Link t, Key k, int *found)
{
	...
	if (t == NULL) {
		...
		*found = 1; // item found, store true  
		...
		t->within->ncompares += 2; // if and elif below
		if (t->left == NULL){
			...
			*found = 1;
			...
			t->within->ncompares += 2; // if and else below
